...
export default function WelcomeScreen({ user }) {
  ...
  return (
    <div className="fixed inset-0 w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800 overflow-hidden dark:bg-gradient-to-br dark:from-gray-900 dark:to-gray-800">
      <div 
        className="absolute inset-0 w-full h-full opacity-50 dark:opacity-50" 
        style={{
      ...
      <div className="absolute w-full h-full overflow-hidden pointer-events-none">
        {/* Floating shapes */}
        <div className={`absolute top-1/4 left-1/4 w-24 h-24 bg-purple-500/20 rounded-xl transform rotate-12 transition-all duration-1000 ${animationReady ? 'opacity-70' : 'opacity-0'}`} 
        ...
        <div className={`absolute top-1/3 right-1/4 w-20 h-20 bg-purple-600/20 rounded-xl transform rotate-45 transition-all duration-1000 delay-500 ${animationReady ? 'opacity-70' : 'opacity-0'}`}
      ...
      <div className={`container mx-auto px-6 z-10 transition-all duration-1000 transform ${animationReady ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
        <div className="flex flex-col lg:flex-row items-center justify-between max-w-6xl mx-auto">
          <div className="lg:w-1/2 mb-12 lg:mb-0 text-center lg:text-left">
            <h1 className="text-5xl md:text-7xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400 mb-6 dark:from-purple-400 dark:to-indigo-400">
              Assistant Code Review
            </h1>
            <p className="text-xl md:text-2xl text-purple-300 mb-8 max-w-xl mx-auto lg:mx-0 dark:text-purple-300">
              ...
              <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6 justify-center lg:justify-start">
                {!user && <button
                  className={`px-8 py-4 text-lg font-medium rounded-xl transition-all duration-300 ${
                    hoveredButton === 'login' 
                      ? 'bg-gradient-to-r from-purple-500 to-purple-700 text-white transform -translate-y-1 shadow-xl shadow-purple-400/30 dark:shadow-purple-400/30' 
                      : 'bg-purple-600 text-white shadow-lg shadow-purple-400/20 dark:shadow-purple-400/20'
                  }`}
                ...
                {user && <button
                  className={`px-8 py-4 text-lg font-medium rounded-xl transition-all duration-300 ${
                    hoveredButton === 'dashboard' 
                      ? 'bg-gradient-to-r from-indigo-500 to-indigo-700 text-white transform -translate-y-1 shadow-xl shadow-indigo-400/30 dark:shadow-indigo-400/30' 
                      : 'bg-indigo-600 text-white shadow-lg shadow-indigo-400/20 dark:shadow-indigo-400/20'
                  }`}
