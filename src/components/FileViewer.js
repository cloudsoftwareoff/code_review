    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          <button
            onClick={onBack}
            className="flex items-center text-violet-700 hover:text-violet-900 font-medium mr-4 transition-colors"
          >
            <svg className="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
            </svg>
          ...
          <h3 className="text-lg font-semibold text-violet-800">{file.name}</h3>
        </div>
        <button
          onClick={fetchAIRecommendations}
          disabled={loadingRecommendations || !fileContent}
          className="flex items-center bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white text-sm py-2 px-4 rounded-lg transition-colors"
        >
      ...
      {recommendations && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-md font-medium text-gray-800">AI Recommendations</h4>
            {!isTypingComplete && (
              <button 
          ...
          <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 shadow-sm prose prose-violet max-w-none">
            {isTypingComplete ? (
              <ReactMarkdown
      ...
      {loadingContent ? (
        ...
        <div className="mt-6">
          <h4 className="text-md font-medium text-gray-800 mb-2">File Content</h4>
